...
def main():
    ...
    Mean_filter(g, g)
    Mean_filter(g_imp, g_imp)
    Mean_filter(g_uni, g_uni)

    plt.show()

def Mean_filter(noise_img, n_img):
    fig, (ax1, ax2, ax3) =plt.subplots(1, 3)
    denoised_img = cv2.blur(noise_img,(15,15))
    ...
    print("psnr Mean filtered image: ", d_psnr)
    print("psnr noise image: ", n_psnr)
    print("ssim Mean filtered image: ", d_score)
    print("ssim noise image: ", n_score)
    print("------------------------------------------")
    
    ax1.imshow(denoised_img, cmap="gray")
    ax1.set_title("Mean Filtered Image")
